...
int *SportsLayout::greedy_hill_climbing(int *old_mapping, int max_iterations, long long *value, timetype initialTime)
{
    ...
    // while (1)
    {
        ...
        auto d = duration(now(), initialTime) if (int(d.count()) / 100000 >= (600.0 * time - 1)) break;
        // // Create a random device to seed the random number generator
        // std::random_device rd;

        // // Create a Mersenne Twister random number generator with the seed from rd
        // std::mt19937 gen(rd());

        // // Create a uniform real distribution between 0.0 (inclusive) and 1.0 (exclusive)
        // std::uniform_real_distribution<double> distribution(0.0, 1.0);

        // // Generate a random number in the [0, 1) range
        // double random_number = distribution(gen);

        // if (random_number < (float)z / (l * l))
        // {
        //     cout << (float)z / (l * l) << endl;
        //     break;
        // }
    }
...
int *SportsLayout::random_restart_greedy_hill_climbing(int max_iterations)
{
    ...
    while (1)
    {
        // {   cout << "Before" << std::to_string(best_value) << endl;
        int *current_mapping = random_mapping();
        long long value = __LONG_LONG_MAX__;
        // auto iTime = now();
        // std::time_t currentTime = std::chrono::system_clock::to_time_t(iTime);
        //     std::cout << "Current time: " << std::ctime(&currentTime);
        int *mapping = greedy_hill_climbing(current_mapping, max_iterations, &value, initialTime);
        // auto fTime = now();
        // std::time_t currentTimeg = std::chrono::system_clock::to_time_t(iTime);
        // std::cout << "Current time: " << std::ctime(&currentTimeg);
        // int value = neighbour_cost_fn(mapping); // THIS IS WRONG IT RETURNS LONG LONG
        //  cout << "Mid" << std::to_string(best_value) << value << neighbour_cost_fn(mapping);
        if (value < best_value)
        {
            delete[] best_mapping;
        ...
        //     cout<<mapping[i]<<" ";
        //     cout<<endl;
        // cout << "After" << std::to_string(best_value) <<neighbour_cost_fn(best_mapping)<< endl;
        auto finalTime = now();
        ...
        if ((double(d.count()) / 100000.0) >= (600.0 * time - maxtimegap / 100000.0 - 1))
        {
...
void SportsLayout::compute_allocation()
{
    ...
    // if (l<250)
    // mapping = random_restart_greedy_hill_climbing(pow(10, 6));
    // else
    mapping = best_neighbour_first_with_restarts(100);
    std::cout << std::endl;
    auto finalTime = now();
}
